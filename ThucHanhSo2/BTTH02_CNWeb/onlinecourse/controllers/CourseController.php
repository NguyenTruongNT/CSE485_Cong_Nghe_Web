...
require_once MODELS_PATH . '/Enrollment.php'; // <--- THÊM DÒNG NÀY
...
class CourseController
{
    ...
    private $lessonModel;
    private $enrollmentModel; // <--- THÊM DÒNG NÀY
    ...
    public function __construct()
    {
        ...
        $this->enrollmentModel = new Enrollment($db); // <--- THÊM DÒNG NÀY
    ...
    // them boi Truong
    // File: controllers/CourseController.php (Kiểm tra và thay thế phương thức register)

    public function register()
    {
        // ... (Kiểm tra POST, session, course_id)
        if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_SESSION['user_id']) || ($_SESSION['user_role'] ?? 2) != 0) {
            header('Location: ' . BASE_URL . '/courses');
            exit();
        }
        $student_id = $_SESSION['user_id'];
        ...
        // ... (Bỏ qua phần xử lý lỗi course_id = 0)

        // Đảm bảo enrollmentModel đã được khởi tạo
        if (!isset($this->enrollmentModel)) {
            require_once MODELS_PATH . '/Enrollment.php';
            $this->enrollmentModel = new Enrollment($this->db);
        }

        // 2. Gọi EnrollmentModel->create()
        try {
            // Hàm create sẽ trả về TRUE nếu thành công, FALSE nếu đã đăng ký (hoặc thất bại khác)
            $is_success = $this->enrollmentModel->create($course_id, $student_id);

            if ($is_success === true) {
                // Đăng ký thành công
                header('Location: ' . BASE_URL . '/course/learn/' . $course_id . '?msg=' . urlencode('✅ Đăng ký khóa học thành công!'));
            } elseif ($is_success === false) {
                // Đã đăng ký (Model trả về false do lỗi 1062 đã được bắt)
                header('Location: ' . BASE_URL . '/courses/detail/' . $course_id . '?msg=' . urlencode('⚠️ Bạn đã đăng ký khóa học này rồi.'));
            } else {
                // Lỗi DB khác (ví dụ: khóa học không tồn tại)
                header('Location: ' . BASE_URL . '/courses/detail/' . $course_id . '?msg=' . urlencode('❌ ĐK thất bại do lỗi hệ thống.'));
            }
        } catch (Exception $e) {
            // Bắt các lỗi ngoại lệ không phải 1062
            header('Location: ' . BASE_URL . '/courses/detail/' . $course_id . '?msg=' . urlencode('❌ Lỗi hệ thống: ' . $e->getMessage()));
        }
        exit();
    }
