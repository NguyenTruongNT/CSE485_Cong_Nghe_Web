...
class Enrollment
{
    ...
    {
        // 1. Kiểm tra nếu học viên đã đăng ký (Sử dụng pre-check)
        if ($this->isEnrolled($course_id, $student_id)) {
            return false; // Trả về false nếu đã đăng ký
        }

        // 2. Chuẩn bị câu lệnh INSERT
        // Giả định bảng có các cột: course_id, student_id, enrolled_date, status
        $query = "INSERT INTO " . $this->table . " 
                  (course_id, student_id, enrolled_date, status) 
                  VALUES (:cid, :sid, NOW(), 'active')";
        ...
        try {
            ...
            // Lệnh execute() là nơi lỗi 1062 đang xảy ra (Dòng 73 của bạn)
            return $stmt->execute();
        } catch (\PDOException $e) {
            // 3. Bắt lỗi 23000 (Integrity constraint violation) và mã lỗi 1062 (Duplicate entry)
            // Nếu lỗi vẫn xảy ra, bắt và trả về false thay vì Fatal Error
            if ($e->getCode() === '23000' && strpos($e->getMessage(), '1062') !== false) {
                // Đã đăng ký (xử lý lỗi 1062 thành công)
                return false;
            }

            // Re-throw (ném lại) các lỗi DB khác không phải lỗi 1062
            throw $e;
        }
    }
